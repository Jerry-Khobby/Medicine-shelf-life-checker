...
const AllMedicine = () => {
  ...
  return (
    <div className='min-h-screen mt-20 flex flex-col px-5 lg:px-8'>
      <div>
        <h2 className='font-semibold font-mono text-xl'>All Medicines Present</h2>
      </div>
      ...
      <div className='flex items-center justify-center'>
        <div className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-4 md:gap-8 lg:gap-4 justify-center items-center mt-7'>
          {data.map((medicine) => (
            <div key={medicine.id} className='border-2 border-gray-200 rounded-lg w-72 sm:w-72 lg:w-56 md:w-72 xlg:w-56 h-80 shadow-sm transition duration-500 ease-in-out hover:scale-105'>
              <div>
                {medicine.imageUrl ? (
                ...
                <div className='mt-8 pb-2'>
                  ...
                  <h6 className='font-normal text-xs text-center'>Expiring Date: {handleDateFormat(medicine.expirationDate)}</h6>
